        ...
        <div class="container-xl px-4 mt-4 main-bg-color">
          <div class="row">
            <div class="custom-col-xl-8">
              ...
              %>
                  ...
                  <div class="card mb-4">
                    ...
                    <form id="addListForm" action="addList.do" method="post" onsubmit="return validateForm()">
					    <div class="card-body">
					        <!-- Form Row-->
					        <div class="row gx-3 mb-3">
					            ...
					            <div class="mb-3">
					                <label class="small mb-1 font-setting" for="festivalSelect">축제명</label>
					                <div name="festival_name">
					                    <select id="festivalSelect" class="form-select" aria-label="Default select example" onchange="updateDateOptions()">
					                        <option value="">-- 축제 선택 --</option>
					                        <c:forEach var="f" items="${selectFestival}">
					                            <c:if test="${loginId==f.admin_id}">
					                                <option value="${f.fes_name}" data-start="${f.fes_start}" data-end="${f.fes_end}">${f.fes_name}</option>
					                            </c:if>
					                        </c:forEach>
					                    </select>
					                </div>
					            ...
					            <div class="col-md-6">
					                <label class="small mb-1 font-setting" for="festivalStartSelect">축제 시작 날짜</label>
					                <div name="festival_name">
					                    <select id="festivalStartSelect" class="form-select" aria-label="Default select example" disabled>
					            ...
					            <!-- Form Group (festival period)-->
					            <div class="col-md-6">
					                <label class="small mb-1 font-setting" for="festivalEndSelect">축제 종료 날짜</label>
					                <div name="festival_name">
					                    <select id="festivalEndSelect" class="form-select" aria-label="Default select example" disabled>
					                        <option value="">-- 날짜 선택 --</option>
					                    </select>
					                </div>
					            </div>
					        ...
					        <div class="mb-3">
					            <label class="small mb-1 font-setting" for="input_bno">사업자번호</label>
					            <input name="input_bno" class="form-control" id="input_bno" type="text" placeholder="지정할 사업자번호를 입력하세요." value="">
					        </div>
					    </div>
					    <!-- Submit button-->
					    <div class="card mb-4">
            ...
            <script>
			    ...
			    // 선택한 축제명에 해당하는 날짜만 나타나게 함.
			    function updateDateOptions() {
			        // 축제명 선택 셀렉트 박스에서 선택한 축제명의 시작 날짜와 종료 날짜를 가져옴
			        var selectedFestival = document.getElementById("festivalSelect");
			        var selectedOption = selectedFestival.options[selectedFestival.selectedIndex];
			        var startDate = selectedOption.getAttribute("data-start");
			        var endDate = selectedOption.getAttribute("data-end");

			        // 시작 날짜 셀렉트 박스를 업데이트
			        var startSelect = document.getElementById("festivalStartSelect");
			        startSelect.innerHTML = "<option value=''>" + ${f.fes_start} + "</option>";
			        startSelect.disabled = false;

			        // 종료 날짜 셀렉트 박스를 업데이트
			        var endSelect = document.getElementById("festivalEndSelect");
			        endSelect.innerHTML = "<option value=''>" + ${f.fes_end} + "</option>";
			        endSelect.disabled = false;

			        // 선택된 축제명을 hidden input에 설정
			        document.getElementById("selectedFestivalName").value = selectedFestival.value;
			        document.getElementById("selectedFestivalStart").value = startDate;
			        document.getElementById("selectedFestivalEnd").value = endDate;
			    }
